...
DATE=$(date +%Y%m%d%H%M)
# 基础路径
BASE_PATH=/work/projects/sky_apps
# 编译后 jar 的地址。部署时，Jenkins 会上传 jar 包到该目录下
SOURCE_PATH=$BASE_PATH/build
# 服务名称。同时约定部署服务的 jar 包名字也为它。
SERVER_NAME=sky_apps_api
# 环境
PROFILES_ACTIVE=qa
# 健康检查 URL
HEALTH_CHECK_URL=http://127.0.0.1:48080/api/health
...
# 日志输出地址
JAVA_OUT=/root/logs/sky_apps_api.log
...
function start() {
    ...
    # 开始启动
    BUILD_ID=dontKillMe nohup java -server $JAVA_OPS $JAVA_AGENT -jar $BASE_PATH/$SERVER_NAME.jar --spring.profiles.active=$PROFILES_ACTIVE > ${JAVA_OUT} 2>&1 &
